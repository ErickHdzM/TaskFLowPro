...
export const create = requireAuth( async (req: AuthRequest, res: Response) => {
    ...
    if (err.length > 0) throw new AppError(400, 'Invalid data structure');

    const project_id = req.params?.project_id as string;
    const user_id = req.user!.id;
    const task = await insertTask({ ...dto, project_id }, user_id);
...
export const update = requireAuth(async (req: AuthRequest, res: Response) => {
    ...
    const task = await updateTask(id, dto, project_id, user_id);
    if (task.affected === 0) throw new AppError(500, 'No data affected');
...
export const changeStatus = requireAuth(async (req: AuthRequest, res:Response) => {
    ...
    const user_id = req.user!.id;
    if (!id) throw new AppError(400, 'Missing information');
    const task = await changeStatusTask(id, dto.status, project_id, user_id);
...
export const drop = requireAuth(async (req: AuthRequest, res: Response) => {
    ...
    const user_id = req.user!.id;
    if (!id) throw new AppError(400, 'Missing Information');
    await dropTask(id, project_id, user_id);
